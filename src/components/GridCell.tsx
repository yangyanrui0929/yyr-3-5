    ...
    >
      ...
      {snapshotCell && snapshotCell.type !== 'empty' && diff?.type === 'removed' && (
        <div className="absolute inset-0 pointer-events-none opacity-50">
          <div className="absolute inset-0 rounded border-2 border-dashed border-indigo-400 bg-indigo-200/30" />
          <div className="absolute inset-0 flex items-center justify-center filter blur-[1px] opacity-70">
      ...
      {snapshotCell && snapshotCell.type !== 'empty' && diff?.type === 'modified' && (
        <div className="absolute top-0 left-0 pointer-events-none">
          <div className="w-2.5 h-2.5 rounded-b-br rounded-tl opacity-80 bg-indigo-400/60 border-b border-r border-indigo-600 border-dashed" />
        </div>
      )}
